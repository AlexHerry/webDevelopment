    ...
    <el-row style="padding:0 20px;margin-top:20px">
      <el-tabs tab-position="left" stretch v-model="active" @tab-click="changePage">
        <el-tab-pane label="基本信息">
          <el-form ref="addGoodsFm" :model="addGoodsForm" class="demo-form-inline" :rules="rule">
            ...
            <el-form-item label="商品分类" prop="goods_cat">
              <br />
              <el-cascader v-model="addGoodsForm.goods_cat" :options="productList" :props="defaultProps" :value="addGoodsForm.goods_cat"></el-cascader>
            </el-form-item>
          </el-form>
        </el-tab-pane>
        ...
        <el-tab-pane label="商品内容">
          ...
          <el-button type="primary" @click="addGoodsData">修改商品</el-button>
        </el-tab-pane>
      </el-tabs>
    </el-row>
  </div>
...
export default {
    ...
    addGoodsData() {
      this.$refs.addGoodsFm.validate((valid) => {
        if (valid) {
          ...
          this.getData(`/goods/${this.$route.params.id}`, 'PUT', this.addGoodsForm, (bak) => {
            console.log(bak);
  ...
  mounted() {
    ...
    this.getData(`/goods/${this.$route.params.id}`, 'GET', '', (bak) => {
      this.addGoodsForm = bak.data;
      this.addGoodsForm.goods_cat = this.addGoodsForm.goods_cat.split(',');
      this.addGoodsForm.pics.forEach((val, index) => {
        this.fileList.push({name: index, url: val.pics_big_url});
      });
    });
  },
};
